...
if [ ! -f ${WRKDIR}/reference-normal_median.rds ]; then
  ...
  ls ${WRKDIR}/controls-BLYMv2/*.wig > normal-wigs.txt
  # Call R and run the script
  Rscript  ${ICHORDIR}/scripts/createPanelOfNormals.R --filelist normal-wigs.txt --gcWig ${ICHORDIR}/inst/extdata/gc_hg19_1000kb.wig --mapWig ${ICHORDIR}/inst/extdata/map_hg19_1000kb.wig --centromere ${ICHORDIR}/inst/extdata/GRCh37.p13_centromere_UCSC-gapTable.txt --outfile ${WRKDIR}/ichor-reference-normal
fi
